...
/**
 * Generated class for the WuxingsanmabudingweiComponent component.
 *
 * See https://angular.io/api/core/Component for more info on Angular
 * Components.
 */
@Component({
  selector: 'wuxingsanmabudingwei',
  templateUrl: 'wuxingsanmabudingwei.html'
})
export class WuxingsanmabudingweiComponent extends commonMethod{
  ...
  constructor(public common:CommonProvider, public tool:ToolsProvider, public util:UtilProvider,public basket:BasketDataProvider){
    ...
    console.log('Hello WuxingsanmabudingweiComponent Component');
  ...
  getCount(){
    ...
    if(tempData.first.length < 3)
        count = 0
    else
        count = this.tool.zuhe1(tempData.first.length,3)
    return count
  }
}
